...
export class LFPScrollNav extends HTMLElement {
    ...
    _$scrollContainer = this.querySelector('[scroll-container]');
    _$scrollTarget;
    _$prevBtn = this.#createBtn(this.getAttribute('previous-text') || '❮', 'afterbegin');
    _$nextBtn = this.#createBtn(this.getAttribute('next-text') || '❯', 'beforeend');
    _hideBtns = ["left", "right", "both", "none"].find(d => d === this.getAttribute("hide-buttons")?.toLowerCase()) || "none";
    _toggleBtns = Number(this.getAttribute('toggle-buttons')) || 10;
    _gap = Number(this.getAttribute('test'));
    _scrollFactor = Number(this.getAttribute('scroll-distance')) || 1;
    _scrollOffset = Number(this.getAttribute('scroll-offset')) || 0;
    _listWidth;
    _inlinePadding;
    constructor() {
        super();
      ...
      ${this._hideBtns !== 'none'
            ? /* css */ `[data-trigger]${this._hideBtns === 'left'
                ? ':first-of-type'
      ...
      [data-trigger] {
        ...
        this._gap = this.#pxStr2Num(this.#getStyle(getComputedStyle(this._$scrollContainer), 'gap'));
    }
    connectedCallback() {
        ...
        if (this._scrollOffset !== 0) {
            ...
            this._$scrollContainer.scroll(offset - (this._hideBtns === 'both' ? 0 : this._gap), 0);
        }
        // get scroll target modifier
        // overwrites scroll offset setting
        const scrollTargetId = this.getAttribute('scroll-target');
        if (scrollTargetId)
            this.#scrollToTarget(idify(scrollTargetId));
        // set up layout
    ...
    attributeChangedCallback(name, _, newVal) {
        switch (name) {
            case 'scroll-target':
                ...
                if (newVal)
                    this.#scrollToTarget(idify(newVal));
                break;
        }
    }
    handleEvent(event) {
        ...
        switch (event.type) {
            ...
            case 'click': {
                ...
                const btn = event.target?.closest('[data-trigger]');
                if (!btn)
                    return;
                // if true, go left, else go right
                const isFirst = btn === this.firstElementChild;
                let scrollDistance = this._$scrollContainer.scrollLeft +
    ...
    #scrollToTarget(id) {
        ...
        this._$scrollTarget = scrollTarget;
     ...
     */
    #isOverflowing() {
        return !(this.getBoundingClientRect().width - this._inlinePadding <
            this._listWidth);
    }
    /**
    ...
    #toggleBtns() {
        ...
        if (this.#isOverflowing()) {
            this._$prevBtn[this._$scrollContainer.scrollLeft >= this._toggleBtns
                ? 'removeAttribute'
                : 'setAttribute']('disabled', '');
            // could use scrollLeftMax but is non-standard
            this._$nextBtn[this._$scrollContainer.scrollLeft <
            ...
            // could use scrollLeftMax but is non-standard
            if (this._$scrollContainer.scrollLeft > this._$scrollContainer.clientWidth ||
